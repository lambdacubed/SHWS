# TODO make it a standard whether they need to have .adf at the end
...
def write_adf(person, filename):
	...
	array = person#.zernike_coefficients
	with open(filename + '.adf', 'w') as fileout:   # open the file to write values to
		...
		for i in range(array.size):     # write each gene to the file
			tsvwriter.writerow([i, array[i]])     # write the index, a tab character, and then the gene's voltage
